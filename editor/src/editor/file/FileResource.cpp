...
namespace gallus::file
{
	...
	//---------------------------------------------------------------------
	std::string FileResource::GetName() const
	{
	...
	//---------------------------------------------------------------------
	void FileResource::SetAssetType(resources::AssetType a_AssetType)
	{
	...
	{
		std::string path = string_extensions::StringToLower(m_Path.filename().generic_string());
		if (path == a_sName)
		{
	...
	{
		if (m_Path == a_Path || GetName() == a_Path)
		{
			a_pFileResource = this;
			return true;
		}
