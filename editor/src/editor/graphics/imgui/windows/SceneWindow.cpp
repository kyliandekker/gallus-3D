	...
	namespace graphics
	{
		namespace imgui
		{
			...
			void SceneWindow::HandleViewportControls()
			{
				...
				graphics::imgui::ActivateInputScope(graphics::imgui::EditorInputScope::EditorInputScope_Viewport, isWindowHovered);
			...
			void SceneWindow::HandleSceneViewControls(double a_fDeltaTime, const ImRect& a_vSceneRect)
			{
				...
				graphics::imgui::ActivateInputScope(graphics::imgui::EditorInputScope::EditorInputScope_SceneHoverRightClick, bHovered && mouseRightDown);
				graphics::imgui::ActivateInputScope(graphics::imgui::EditorInputScope::EditorInputScope_SceneHover, bHovered);
				...
				win32::Window* window = GetEngine().GetWindow();
				if (!window)
				{
					return;
				}
				...
				if (mouseRightDown)
				{
					if (s_bFirstFrame)
					{
						...
						IVector2 centerPos;
						centerPos.x = static_cast<int>((a_vSceneRect.Min.x + a_vSceneRect.Max.x) * 0.5f);
						centerPos.y = static_cast<int>((a_vSceneRect.Min.y + a_vSceneRect.Max.y) * 0.5f);

						IVector2 windowPos = window->GetPosition();
						s_CenterPos.x = static_cast<int>(windowPos.x + centerPos.x);
						s_CenterPos.y = static_cast<int>(windowPos.y + centerPos.y);
					}

					IVector2 currentPos = window->GetCursorPosition();
					float deltaX = static_cast<float>(currentPos.x - s_CenterPos.x);
					float deltaY = static_cast<float>(currentPos.y - s_CenterPos.y);
					window->SetCursorPosition({ s_CenterPos.x, s_CenterPos.y });
