...
namespace gallus
{
	namespace graphics
	{
		namespace imgui
		{
			...
			{
				float scrolledFrameWidth = ANIMATION_FRAME_PIXEL_WIDTH * m_fScroll;
				...
				))
				{
					...
					ImRect legendRect = {
						initialPos,
						initialPos + ImVec2((frameCount * scrolledFrameWidth) + (legendPadding.x * 2), LEGEND_HEIGHT)
					};
					...
					))
					{
						...
						// --- Legend ticks ---
						{
							...
							for (int i = frameMin; i <= frameCount; i += frameStep)
							{
								...
								float px = timeLineStart.x + ((i - frameMin) * scrolledFrameWidth);
					...
					))
					{
						...
						for (animation::AnimationKeyFrame* keyFrame : m_Animation.GetKeyFrames())
						{
							...
							float px = timeLineStart.x + (keyFrame->GetFrame() * scrolledFrameWidth);
							px -= iconSize.x / 2;
					...
					// --- Tracker ---
					float selectedFramePos = timeLineStart.x + ((m_iSelectedFrame - frameMin) * scrolledFrameWidth);
					...
					if (ImGui::IsMouseHoveringRect(legendRect.Min, keyFrameTimelineRect.Max) && bMouseDown)
					{
						...
						int index = std::floor(relativePos.x / scrolledFrameWidth);
				...
				//	if (fWheel != 0.0f)
				//	{
				//		float fOldZoom = m_fScroll;
				//		m_fScroll *= (fWheel > 0.0f) ? 1.1f : 0.9f;
				//		m_fScroll = std::clamp(m_fScroll, 0.2f, 5.0f);
