...
namespace gallus::graphics::imgui
{
	...
	public:
		...
		StringDropdown(ImGuiSystem& a_System) : ImGuiUIView(a_System)
		{}
		...
		{
			m_iSelected = std::distance(m_aOptions.begin(), std::find(m_aOptions.begin(), m_aOptions.end(), a_Value));
		}
		...
		bool Render(const char* a_Label)
		{
			...
			{
				for (size_t n = 0; n < m_aOptions.size(); n++)
				{
					bool is_selected = (m_iSelected == n);
					if (ImGui::Selectable(ToString(m_aOptions[n]).c_str(), is_selected))
					{
						m_iSelected = n;
						b = true;
					}
	...
	private:
		std::vector<T> m_aOptions; /// The options to display.
		std::function<std::string(T)> ToString; /// Function pointer that converts the option to a string.
		size_t m_iSelected = 0; /// The index of the currently selected option.
	};
}