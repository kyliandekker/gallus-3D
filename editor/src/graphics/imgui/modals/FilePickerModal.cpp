...
namespace gallus
{
	namespace graphics
	{
		namespace imgui
		{
			...
			bool FilePickerModal::Initialize()
			{
				...
				m_pPreviewTexture = core::TOOL->GetResourceAtlas().LoadTexture("tex_missing.png", cCommandList);
				m_pPreviewTexture->SetResourceCategory(gallus::core::EngineResourceCategory::Editor);
				m_pPreviewTexture->SetIsLocked(true);
				m_pPreviewTexture->SetIsUnique(true);
