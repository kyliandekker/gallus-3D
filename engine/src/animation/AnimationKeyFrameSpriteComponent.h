	...
	namespace animation
	{
		...
		private:
			...
			BEGIN_EXPOSE_FIELDS_PARENT(AnimationKeyFrameSpriteComponent, AnimationKeyFrameComponentBase)
				EXPOSE_FIELD(AnimationKeyFrameSpriteComponent, m_pSprite, "Sprite", "Pointer to the texture asset used by this sprite. Can be nullptr if no texture is assigned. Determines the visual appearance of the sprite.",
					(FieldOptions{
						.type = EditorFieldWidgetType::EditorFieldWidgetType_EngineResource,
						.assetType = resources::AssetType::Sprite,
						}))
						EXPOSE_FIELD(AnimationKeyFrameSpriteComponent, m_iSpriteIndex, "Sprite Index", "Index of the sprite within a texture atlas. Used when the texture contains multiple sprites to select which one is displayed.",
							(FieldOptions{
