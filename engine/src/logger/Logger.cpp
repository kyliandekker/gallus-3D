...
namespace gallus::logger
{
	...
	// LoggerMessage
	//---------------------------------------------------------------------
	LoggerMessage::LoggerMessage(const std::string& a_sRawMessage, const std::string& a_sCategory, const std::string& a_Location, uint32_t a_iLine, LogSeverity a_Severity, const std::chrono::system_clock::time_point& a_Time) :
	...
	bool Logger::InitThreadWorker()
	{
		...
		if (m_bLogToFile)
		{
			...
			if (!s_pLogFile)
			{
				LOG(LOGSEVERITY_SUCCESS, LOG_CATEGORY_LOGGER, "Failed initializing logger: Could not create log file.");
				return false;
			}
		}

		LOG(LOGSEVERITY_SUCCESS, LOG_CATEGORY_LOGGER, "Successfully initialized logger.");
	...
	void Logger::Loop()
	{
		...
		while (!m_Messages.empty())
		{
			...
			fileName = filePath.parent_path().filename().generic_string() + "/" + filePath.filename().generic_string();
	...
	bool Logger::Destroy()
	{
		LOG(LOGSEVERITY_INFO, LOG_CATEGORY_LOGGER, "Destroying logger.");
		return ThreadedSystem::Destroy();
	}

	//---------------------------------------------------------------------
	void Logger::Log( LogSeverity a_Severity, const std::string& a_sCategory, const std::string& a_sMessage, const std::string& a_sFile, int a_iLine)
	{
		PrintMessage(a_Severity, a_sCategory, a_sMessage, a_sFile, a_iLine);
	}

	//---------------------------------------------------------------------
	void Logger::LogF(LogSeverity a_Severity, const std::string& a_sCategory, const std::string& a_sMessage, const std::string& a_sFile, int a_iLine, ...)
	{
	...
	//---------------------------------------------------------------------
	void Logger::PrintMessage(LogSeverity a_Severity, const std::string& a_sCategory, const std::string& a_sMessage, const std::string& a_sFile, int a_iLine)
	{
