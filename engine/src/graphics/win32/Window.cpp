...
namespace gallus::graphics::win32
{
	...
	//---------------------------------------------------------------------
	void Window::SetCursorPosition(const IVector2& a_vPos)
	{
		SetCursorPos(a_vPos.x, a_vPos.y);
	}

	//---------------------------------------------------------------------
	IVector2 Window::GetCursorPosition()
	{
		POINT currentPos;
	...
	bool Window::InitThreadWorker()
	{
		...
		if (m_WindowSettings.Load())
		{
			LOG(LOGSEVERITY_SUCCESS, LOG_CATEGORY_WINDOW, "Successfully loaded window settings.");
		}
		SetSize(m_WindowSettings.GetSize());
		SetPosition(m_WindowSettings.GetPosition());

		LOG(LOGSEVERITY_SUCCESS, LOG_CATEGORY_WINDOW, "Successfully initialized window.");
	...
	void Window::Finalize()
	{
		...
		LOG(LOGSEVERITY_SUCCESS, LOG_CATEGORY_WINDOW, "Successfully destroyed window.");
	}
	...
	bool Window::CreateWindow(HINSTANCE a_hInstance)
	{
		...
		{
			LOG(LOGSEVERITY_ASSERT, LOG_CATEGORY_WINDOW, "Failed creating window: Failed window registration.");
			return false;
		}
		...
		if (!m_hWnd)
		{
			LOG(LOGSEVERITY_ASSERT, LOG_CATEGORY_WINDOW, "Failed creating window: Hwnd was null.");
			return false;
		}
