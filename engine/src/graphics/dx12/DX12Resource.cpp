...
namespace gallus::graphics::dx12
{
	...
	//---------------------------------------------------------------------
    bool DX12Resource::SetResourceData(const std::string& a_sName, const D3D12_RESOURCE_DESC& a_ResourceDesc, const D3D12_HEAP_PROPERTIES& a_Heap, const D3D12_RESOURCE_STATES a_ResourceState, const D3D12_CLEAR_VALUE* a_pOptimizedClearValue)
    {
		...
		{
			LOGF(LOGSEVERITY_ERROR, LOG_CATEGORY_DX12, "Failed setting resource data: Failed creating committed resource: \"%s\".", a_sName.c_str());
			return false;
		}

		if (!m_pResource)
		{
			LOGF(LOGSEVERITY_ERROR, LOG_CATEGORY_DX12, "Failed setting resource data: Resource was null: \"%s\".", a_sName.c_str());
			return false;
		}

		std::wstring name = std::wstring(a_sName.begin(), a_sName.end());
