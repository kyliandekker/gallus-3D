...
namespace gallus::graphics::dx12
{
		...
		m_iFenceValue(0)
	{
		...
		{
			LOG(LOGSEVERITY_ERROR, LOG_CATEGORY_DX12, "Failed creating command queue: Failed creating fence.");
			return;
		}
		...
		if (!m_FenceEvent)
		{
			LOG(LOGSEVERITY_ERROR, LOG_CATEGORY_DX12, "Failed creating command queue: Failed creating fence event.");
			return;
		}
	}
