...
namespace gallus::graphics::dx12
{
	...
	void DirectionalLight::Bind(std::shared_ptr<CommandList> a_CommandList)
	{
		if (!m_pResource)
		{
			SetResourceData("Directional Light", CD3DX12_RESOURCE_DESC::Buffer(sizeof(DirectionalLightData)), CD3DX12_HEAP_PROPERTIES(D3D12_HEAP_TYPE_UPLOAD), D3D12_RESOURCE_STATE_GENERIC_READ, nullptr);
			return;
		}
