...
namespace gallus::graphics::dx12
{
	...
	HeapAllocation::HeapAllocation(const D3D12_DESCRIPTOR_HEAP_DESC& a_Desc) : m_Type(a_Desc.Type)
	{
		...
		if (FAILED(hr))
		{
			LOG(LOGSEVERITY_ERROR, LOG_CATEGORY_DX12, "Failed heap allocation: Failed to create descriptor heap.");
			return;
		}
	...
	size_t HeapAllocation::Allocate()
	{
		...
		LOG(LOGSEVERITY_ERROR, LOG_CATEGORY_DX12, "Failed heap allocation: No available descriptor heap slots!");
	...
	void HeapAllocation::Deallocate(size_t a_iIndex)
	{
		...
		{
			LOG(LOGSEVERITY_WARNING, LOG_CATEGORY_DX12, "Failed heap deallocation: Deallocating index out of bounds.");
			return;
		}
