	...
	namespace graphics
	{
		namespace imgui
		{
			...
			public:
				...
				void SetValue(const T& a_vValue)
				{
					m_vValue = a_vValue;
				}
				...
				virtual uint8_t Render(const char* a_sLabel, float a_fSpeed = 0.05f, float a_fMin = std::numeric_limits<float>::min(), float a_fMax = std::numeric_limits<float>::max())
				{
					...
					bool changedValueX = ImGui::DragFloat(ImGui::IMGUI_FORMAT_ID("", INPUT_ID, std::string("X_") + a_sLabel).c_str(), &m_vValue.x, a_fSpeed, a_fMin, a_fMax);
					
					ImGui::AlignTextToFramePadding();
					ImGui::Text("Y");
					ImGui::SameLine();
					ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
					bool changedValueY = ImGui::DragFloat(ImGui::IMGUI_FORMAT_ID("", INPUT_ID, std::string("Y_") + a_sLabel).c_str(), &m_vValue.y, a_fSpeed, a_fMin, a_fMax);

					return changedValueX ? 1 : (changedValueY ? 2 : 0);
				}
			protected:
				T m_vValue; /// The current value of the vector.
			};
			...
			public:
				...
				uint8_t Render(const char* a_sLabel, float a_fSpeed = 0.05f, float a_fMin = -90000, float a_fMax = std::numeric_limits<float>::max()) override
				{
					...
					ImGui::SameLine();
					ImGui::SetNextItemWidth(ImGui::GetContentRegionAvail().x);
					bool changedValueZ = ImGui::DragFloat(ImGui::IMGUI_FORMAT_ID("", INPUT_ID, std::string("Z_") + a_sLabel).c_str(), &this->m_vValue.z, a_fSpeed, a_fMin, a_fMax);
